...
export function CoinConvertor({ selling, sellingCoin, buyingCoin, sellingAmount, buyingAmount, handleSellingCoin, handleBuyingCoin, handleSellingAmount, handleBuyingAmount}: { selling: boolean , sellingCoin: any , buyingCoin: any, sellingAmount: any, buyingAmount: any,handleSellingCoin: any, handleBuyingCoin: any, handleSellingAmount: any, handleBuyingAmount: any}) {
    ...
    return (
        ...
        <div>
            ...
            <div className="flex justify-between items-center relative mb-6">
                ...
                <input onChange={selling ? (e) => handleSellingAmount(e, sellingCoin, buyingCoin) :  (e) => handleBuyingAmount(e, buyingCoin, sellingCoin)} className="bg-[#191932] text-end max-w-[40%] focus:outline-0" value={selling ? sellingAmount : buyingAmount}/>
                </>
                <ul className={`bg-[#232336] absolute w-full rounded-xl z-10 px-6 top-7 transition-all ${showCoinList ? "opacity-100" : "opacity-0"}`}>
                    {showCoinList && coinList.map((coin) => <li className="cursor-pointer" onClick={() => selling ? handleSellingCoin(coin) : handleBuyingCoin(coin)} key={coin.id}>{coin.name}</li>)}
                </ul>
            </div>
            <div>
            <hr></hr>
